...
const TEXT = {
    en: {
        ...
        mod:             (_n, _id) => `This page is a community-made mod`,
        video:           (_n, _id) => `This page is a Steam video`,
        unavailable:     (name, id) => `Game ${name || "(unnamed)"} with id ${id} is no longer available on Steam`,
        comingSoon:      (name, id) => `Game ${name || "(unnamed)"} with id ${id} is not yet released on Steam`,
        duplicate:       (name, _id) => `${name || "Game"} is already in the queue`,
        masterDuplicate: (name, id) => `Game ${name || "(unnamed)"} with id ${id} is already in the master database`,
    ...
    vi: {
        ...
        mod:             (_n, _id) => `Trang này là mod do cộng đồng tạo`,
        video:           (_n, _id) => `Trang này là video trên Steam`,
        unavailable:     (name, id) => `Game ${name || "(không tên)"} (id ${id}) không còn khả dụng trên Steam`,
        comingSoon:      (name, id) => `Game ${name || "(không tên)"} (id ${id}) chưa phát hành trên Steam`,
        duplicate:       (name, _id) => `${name || "Game"} đã có trong hàng đợi`,
        masterDuplicate: (name, id) => `Game ${name || "(không tên)"} (id ${id}) đã có trong cơ sở dữ liệu chính`,
